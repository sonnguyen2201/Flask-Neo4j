...
def register():
    if request.method == 'POST':
        ...
        if len(username) < 1:
            ...
            flash('Your password must be at least 5 character.')
...
def profile(username):
    ...
    user_begin_viewed = User(user_being_viewed_username)
    posts = user_begin_viewed.get__recent_posts()
    ...
    if logged_in_username:
        ...
        if logged_in_user.username == user_begin_viewed.username:
            similar = logged_in_user.get_similar_users()
        else:
            common = logged_in_user.get_commonality_of_user(user_begin_viewed)
